...
interface ModalDeleteTaskProps {
    ...
    open: boolean;
    close: () => void;
    onDelete: ({ id_tarea }: { id_tarea: string }) => void;
}

export function ModalDeleteTask({ task, open, close, onDelete }: ModalDeleteTaskProps) {
    return (
        <AnimatePresence>
            {open && task ? (
                <motion.div
                ...
                >
                    ...
                    >
                        <h3 className="mb-3 text-lg font-semibold">Confirmación de eliminación</h3>

                        <p className="text-slate-300">
                            ¿Está seguro de que desea eliminar la tarea <strong>{task.titulo}</strong>?
                        </p>
                        ...
                        <div className="mt-4 flex flex-wrap gap-2">
                            ...
                            <motion.button
                                ...
                                onClick={() => onDelete({ id_tarea: task.id_tarea })}
                            >
                                Eliminar
                            </motion.button>
                        </div>
